...
(async()=>{
  ...
  await check('eleven supplied visual references coexist with the original planning artwork',()=>{const refs=s.veluneVisualAssets(),p=s.newStudioProject('VELUNE','velune');assert.equal(refs.length,11);assert.equal(new Set(refs.map(r=>r.id)).size,11);for(const asset of refs){assert(p.assets.some(a=>a.id===asset.id));assert(fs.existsSync(path.join(root,'public',asset.url)));assert.equal(asset.metadata.provenance,'AI-generated reference supplied by Ajwad');}assert(p.assets.some(a=>a.id==='velune-packaging'));assert(p.assets.some(a=>a.id==='velune-report'));});
  await check('Ad example binds all references in declared order and flags older project omissions',()=>{const ad=load('src/lib/veluneAdExample.ts'),draft=ad.veluneAdExample(s.newStudioProject('VELUNE','velune').assets),helpers=load('src/lib/adDraft.ts');assert(helpers.parseAdDraft(draft));assert.equal(draft.references.length,12);assert.equal(draft.references.filter(r=>r.kind==='image').length,11);assert.equal(helpers.referenceBindingProblems(draft.references,draft.referenceManifest).length,0);assert(!draft.references.some(r=>r.role==='character'));assert(!draft.references.some(r=>r.id==='velune-report-v2'));const old=ad.veluneAdExample(s.veluneAssets().filter(a=>!a.id.startsWith('velune-ref-')));assert.equal(old.unattachedSlots.length,11);assert.equal(helpers.referenceBindingProblems(old.references,old.referenceManifest).length,11);assert(draft.sceneCards.find(s=>s.id==='S07').referenceIds[1]==='velune-ref-v2-studio');});
