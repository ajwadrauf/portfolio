...
(async () => {
  ...
  await reject({ ...body, referenceImageDataUrls: body.referenceImageDataUrls.slice(0, 1), referenceVideoUrls: ['https://media.example/a.mp4', 'https://media.example/b.mp4'], referenceVideoDurations: [8, 8] }, '15 seconds of video references combined');
  ...
  await reject({ ...body, referenceImageDataUrls: body.referenceImageDataUrls.slice(0, 1), referenceAudioUrls: ['https://media.example/a.mp3'], referenceAudioDurations: [1] }, '2–15 seconds');
  await reject({ ...body, referenceImageDataUrls: body.referenceImageDataUrls.slice(0, 1), referenceAudioUrls: ['https://media.example/a.mp3', 'https://media.example/b.mp3'], referenceAudioDurations: [10, 6] }, '15 seconds of audio references combined');
  ...
  check(response.status === 200 && queued.at(-1).input.reference_image_urls.length === 8, 'Original saved reference URLs remain allowlisted and measurable');
  check(queued.at(-1).input.reference_image_urls.every((url, i) => url.endsWith(legacyRefs[i].url)), 'Old files are not silently replaced by the v2 assets');
