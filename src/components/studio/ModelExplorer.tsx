...
export function ModelExplorer() {
  ...
  return <section className="my-10 rounded-md border border-border-soft bg-surface p-5 sm:p-7" aria-labelledby="model-explorer">
    ...
    <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      {kind === "video" && <><label className="text-sm">Aspect<select className="input mt-1" value={scenario.aspect} onChange={(e) => set("aspect", e.target.value)}>{["9:16", "16:9", "1:1", "4:3", "3:4", "21:9"].map((a) => <option key={a}>{a}</option>)}</select></label><label className="text-sm">Seedance resolution<select className="input mt-1" value={scenario.resolution} onChange={(e) => set("resolution", e.target.value as ModelScenario["resolution"])}>{["480p", "720p", "1080p"].map((r) => <option key={r}>{r}</option>)}</select></label><label className="text-sm">Total motion-guide seconds<input className="input mt-1" type="number" min={0} max={30} step={.5} value={scenario.inputVideoSeconds} onChange={(e) => set("inputVideoSeconds", Number(e.target.value))} /></label></>}
    </div>
