...
export function StudioStart() {
  ...
  return <section className="mx-auto max-w-6xl px-6 pb-12" aria-labelledby="studio-start">
    ...
    <div className="mt-5 grid items-center gap-5 rounded-md bg-[#241922] p-6 text-[#faf6ef] md:grid-cols-[1fr_auto]"><div><p className="label-sm !text-[#d9bcca]">Explore a worked example</p><h3 className="mt-2 text-xl">VELUNE. Follow one brief through the studio.</h3><p className="mt-2 max-w-2xl text-sm leading-relaxed text-[#d7cbd2]">Load the packaging artwork, visual references and Blender camera plan behind the 15-second H3 Max film. Explore the picture prompt and ElevenLabs sound cues, then open the case study to see the film and campaign adaptations.</p><Link className="mt-3 inline-flex min-h-10 items-center text-sm underline underline-offset-4" href="/velune">See the VELUNE case study ↗</Link></div><button className="rounded border border-[#bc9fb0] px-5 py-3 text-sm font-semibold transition hover:bg-white/10 disabled:opacity-50" disabled={busy || !ready} onClick={async () => { setBusy(true); setError(""); try { await createProject("VELUNE · guided example", "velune"); router.push("/ai-studio/projects"); } catch (e) { setError(e instanceof Error ? e.message : "The example could not load."); } finally { setBusy(false); } }}>{busy ? "Preparing example…" : "Load & explore VELUNE ↗"}</button></div>
