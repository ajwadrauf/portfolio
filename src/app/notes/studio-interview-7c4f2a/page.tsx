      ...
      <div className={styles.layout}>
        ...
        <main className={styles.main}>
          ...
          <section id="role" className={styles.section}>
            ...
            <div className={styles.note}><h3>A completed delivery set to show</h3><p>VELUNE now includes a final MP4 with sound and typography, a campaign still, a vertical social layout and an English/French still. Show how the composition changes while the product stays consistent. The final film uses local carton replacements and a typeset report; the original H3 take and separate voiceover remain available for comparison.</p></div>
          </section>
          ...
          <section id="evidence" className={styles.section}>
            ...
            <div className={styles.storyBank}>{stories.map((story, i) => <article className={styles.storyCard} key={story.title}><p className={styles.eyebrow}>Story {i + 1} · {story.source}</p><h3>{story.title}</h3>{"spoken" in story && story.spoken && <blockquote>{story.spoken}</blockquote>}<dl><dt>Situation</dt><dd>{story.situation}</dd><dt>Your action</dt><dd>{story.action}</dd><dt>Result</dt><dd>{story.result}</dd><dt>What it taught you</dt><dd>{story.lesson}</dd></dl><p className={styles.prepare}><strong>Prepare this detail:</strong> {story.prepare}</p></article>)}</div>
          </section>
          ...
          <section id="practice" className={styles.section}>
            ...
            <p>Use the notes to complete the two people stories: one production issue from your conversation with Dasha and the studio decision it informed; one colleague you coached, what you adapted and what they could do afterwards. A specific moment will make each story your own.</p>
            <InterviewPractice questions={questions} />
            <div className={styles.nextPass}><h3>Details to bring into the next practice</h3><p>Complete the Dasha and coaching examples, then add one real disagreement, defensible workflow metrics if available, your exact AI-work timeline, and the interview length and panel roles when you know them.</p></div>
          </section>

          <footer className={styles.sources}><p className={styles.eyebrow}>Basis of this notebook</p><p>Prepared from your supplied AI Content Studio job description, AjwadRauf_Resume.pdf, your own letter to Nan, the decisions observed during our portfolio build, and your account of meeting Dasha to understand production issues. The letter is your application narrative, not an external recommendation. The opening also draws on the themes of ownership, judgment and taste in the excerpt you supplied. Its events come from your VELUNE build, not the writer’s hiring anecdote. Scripts and the 90-day plan are coaching drafts; suggested activities are not claims of past achievements.</p><p>General interview references: <a href="https://careerservices.fas.harvard.edu/resources/interviewing/">Harvard Mignone Center for Career Success</a> and <a href="https://www.opm.gov/policy-data-oversight/assessment-and-selection/structured-interviews/">US Office of Personnel Management</a>. The original attachments and their contact details are not published here.</p><a href="#start">Back to the central story ↑</a></footer>
        </main>
      </div>
    </div>
  );
