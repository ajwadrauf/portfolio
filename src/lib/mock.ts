...
export function mockImageDataUrl(opts: {
  ...
  <defs>
    ...
    <radialGradient id="s" cx="0.5" cy="0.42" r="0.55">
      <stop offset="0" stop-color="#763d5d" stop-opacity="0.14"/>
      <stop offset="1" stop-color="#763d5d" stop-opacity="0"/>
    </radialGradient>
  </defs>
  ...
  <g transform="translate(${w / 2} ${h / 2})">
    ...
    <rect x="${-w * 0.1}" y="${-h * 0.12}" width="${w * 0.2}" height="${h * 0.1}" rx="8" fill="#763d5d" opacity="0.75"/>
  </g>
  <text x="${w / 2}" y="${h - 96}" text-anchor="middle" font-family="system-ui,sans-serif"
    font-size="${Math.round(w * 0.032)}" fill="#272726" font-weight="700">${escapeXml(opts.label)}</text>
