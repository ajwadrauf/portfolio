...
export function veluneAdExample(assets: StudioAsset[]): AdLabSeed {
  ...
  return {
    ...
    sceneCards: VELUNE_GENERATION_SHOTS.map((shot) => {
      ...
      return { id: shot.id, title: shot.title, start: shot.start / 24, end: shot.end / 24, action: videoPromptFor(H3_MODEL_ID, shot.note), camera: shot.id === "S07" ? "Camera pullback from the actual Blender study" : (["S01", "S12"].includes(shot.id) ? "Use the revised still framing; retain the original cut timing" : "Original Blender timing with the specified appearance and action overrides"), sound: [...voice, ...effects, "Separate 80 BPM instrumental score; replace H3 native audio in the final edit."].join(" · "), referenceIds: ["velune-motion", ...ids] };
    }),
  };
}
